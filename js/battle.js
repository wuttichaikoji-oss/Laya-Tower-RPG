...
(function () {
  ...
  const miniBosses = [
    { id:"wolf", name:"Abyss Fang", sprite:"assets/sprites/wolf.svg", element:"Shadow", kind:"mini" },
    { id:"bloom", name:"Mire Basilisk Bloom", sprite:"assets/sprites/bloom.svg", element:"Poison", kind:"mini" },
    { id:"golem", name:"Coral Sentinel", sprite:"assets/sprites/golem.svg", element:"Water", kind:"mini" }
  ];
  const bosses = [
    { id:"warden", name:"Tower Warden", sprite:"assets/sprites/warden.svg", element:"Holy", kind:"boss" },
    { id:"warden", name:"Infernal Archivist", sprite:"assets/sprites/warden.svg", element:"Fire", kind:"boss" },
    { id:"warden", name:"Phantom Queen", sprite:"assets/sprites/warden.svg", element:"Ghost", kind:"boss" }
  ];
  ...
  function pickEnemy(floor) {
    ...
    return {
      type, floor, id: base.id, name: base.name, sprite: base.sprite, element: base.element,
      ...
      statuses: [], enrage: false, intent: null
    };
  }
  ...
  function renderStatuses() {
    document.getElementById("heroStatusRow").innerHTML = state.hero.statuses.map(statusPill).join("") || `<span class="status-pill neutral">No Status</span>`;
    document.getElementById("enemyStatusRow").innerHTML = state.enemy.statuses.map(statusPill).join("") || `<span class="status-pill neutral">No Status</span>`;
  }
  function statusPill(s) {
    const label = s.name === "shield" ? `Shield ${s.turns}` : s.name === "regen" ? `Regen ${s.turns}` : s.name === "poison" ? `Poison ${s.turns}` : s.name === "burn" ? `Burn ${s.turns}` : s.name === "exposed" ? `Exposed ${s.turns}` : s.name === "arcaneBreak" ? `Arcane Break ${s.turns}` : s.name === "corrode" ? `Corrode ${s.turns}` : s.name === "slow" ? `Slow ${s.turns}` : `${s.name} ${s.turns}`;
    const cls = ["shield","regen"].includes(s.name) ? "good" : ["poison","burn","exposed","arcaneBreak","corrode","slow"].includes(s.name) ? "bad" : "neutral";
    return `<span class="status-pill ${cls}">${label}</span>`;
  }

  function addLog(text) {
    const log = document.getElementById("battleLog");
    const line = document.createElement("div"); line.className = "log-line"; line.textContent = text; log.prepend(line);
  }
  function showCenter(text) { document.getElementById("battleCenterText").textContent = text; }

  function popup(targetId, amount, flavor = "") {
    const target = document.getElementById(targetId); const pop = document.createElement("div"); pop.className = `damage-popup ${flavor}`; pop.textContent = flavor === "heal" ? `+${amount}` : `-${amount}`; target.appendChild(pop); setTimeout(() => pop.remove(), 1000);
  }
  ...
  function applyStatus(target, status) {
    if (!status) return;
    const found = target.statuses.find(s => s.name === status.name);
    if (found) { Object.assign(found, status); }
  ...
  function tickStatusPhase(unit, targetId, unitLabel) {
    ...
    unit.statuses.forEach(status => {
      if (status.name === "poison") {
        const dmg = status.power || 8; unit.hp -= dmg; popup(targetId, dmg, "weak"); ringFx(targetId, "poison"); msg.push(`${unitLabel} ถูก Poison ${dmg}`);
      }
      if (status.name === "burn") {
        const dmg = status.power || 7; unit.hp -= dmg; popup(targetId, dmg, "critical"); ringFx(targetId, "fire"); msg.push(`${unitLabel} ถูก Burn ${dmg}`);
      }
      if (status.name === "regen") {
        const heal = status.power || 10; unit.hp = Math.min(unit.maxHp, unit.hp + heal); popup(targetId, heal, "heal"); ringFx(targetId, "heal"); msg.push(`${unitLabel} ฟื้นจาก Regen ${heal}`);
      }
      status.turns -= 1;
    });
  ...
  function activeAmp(defender) {
    let amp = 0;
    defender.statuses.forEach(s => {
      if (s.name === "exposed") amp += s.amp || 0.18;
      if (s.name === "arcaneBreak") amp += s.amp || 0.18;
      if (s.name === "slow") amp += s.amp || 0.1;
    });
    return amp;
  }
  function shieldReduction(defender) {
    let reduce = 0;
    defender.statuses.forEach(s => { if (s.name === "shield") reduce = Math.max(reduce, s.reduce || 0.22); });
    return reduce;
  }
  function corrodePenalty(defender) {
    let shred = 0;
    defender.statuses.forEach(s => { if (s.name === "corrode") shred += s.shred || 3; });
    return shred;
  }
  ...
  function planEnemyIntent() {
    ...
    if (roll < 0.18 && !enemy.statuses.find(s => s.name === "shield")) enemy.intent = { type: "guard", label: "Guard: ลดดาเมจ" };
    else if (roll < 0.45) enemy.intent = { type: "special", label: enemy.element === "Poison" ? "Toxic Bite" : enemy.element === "Fire" ? "Flame Claw" : "Heavy Strike" };
    else enemy.intent = { type: "attack", label: "Basic Attack" };
    text("enemyIntentText", enemy.intent.label);
  }

  function updateIntentText() {
    text("enemyIntentText", state.enemy.intent ? state.enemy.intent.label : "Unknown");
  }
  ...
  function rewardSave(win) {
    ...
    if (win) {
      ...
      while (save.exp >= save.level * 80) {
        save.level += 1; save.maxHp += 18; save.maxMp += 8; save.atk += 4; save.def += 2; save.spd += 1; save.hp = save.maxHp; save.mp = save.maxMp; leveled = true; addLog(`${save.name} เลเวลอัปเป็น ${save.level}!`);
      }
      saveFn(save);
      return { rewardGold, rewardExp, leveled };
    } else {
      save.hp = save.maxHp; save.mp = save.maxMp; saveFn(save); return null;
    }
  }
  ...
  function postActionCheck() {
    ...
    if (state.enemy.hp <= 0) { state.enemy.hp = 0; updateBars(); endBattle(true); return true; }
    if (state.hero.hp <= 0) { state.hero.hp = 0; updateBars(); endBattle(false); return true; }
    return false;
  }

  function startHeroTurn() {
    state.turn += 1; setTimeline("hero"); showCenter(`Turn ${state.turn} • Your Turn`); tickStatusPhase(state.hero, "heroSpriteWrap", state.hero.name); if (postActionCheck()) return; planEnemyIntent(); updateIntentText(); lockActions(false);
  }
  ...
  function enemyTurn() {
    ...
    setTimeout(() => {
      ...
      if (enemy.intent?.type === "enrage") {
        ...
        animateAttack("enemySpriteWrap", "heroSpriteWrap"); state.hero.hp -= result.total; popup("heroSpriteWrap", result.total, result.crit ? "critical" : result.mult > 1 ? "weak" : ""); ringFx("heroSpriteWrap", enemy.element === "Fire" ? "fire" : enemy.element === "Poison" ? "poison" : "");
        addLog(`${enemy.name} ใช้ ${enemy.intent?.label || "โจมตี"} ใส่ ${state.hero.name} ${result.total} ดาเมจ (${result.label})`);
  ...
  function doHeroHit(result, skill, kind = "attack") {
    ...
    if (skill?.apply) applyStatus(state.enemy, skill.apply);
    if (skill?.applySelf) applyStatus(state.hero, skill.applySelf);
  ...
  function basicAttack() {
    if (actionLocked) return; lockActions(true);
    const extraCrit = state.hero.classId === "striker" ? 0.08 : state.hero.classId === "host-ranger" ? 0.06 : 0;
    const result = calcDamage(state.hero, state.enemy, 1.1, extraCrit);
    doHeroHit(result, null, "attack");
  }
  ...
  function useSkill(skill) {
    ...
    if (skill.type === "buff") {
      if (skill.applySelf) applyStatus(state.hero, skill.applySelf);
      state.hero.defBuff = Math.max(state.hero.defBuff || 0, Math.round(state.hero.def * (skill.defendBoost || 0.3))); addLog(`${state.hero.name} ใช้ ${skill.name} เพิ่มพลังป้องกันชั่วคราว`); ringFx("heroSpriteWrap", "magic"); renderStatuses(); setTimeout(enemyTurn, 620); return;
    }
    ...
    doHeroHit(result, skill, skill.type);
  }

  function defend() {
    if (actionLocked) return; applyStatus(state.hero, { name: "shield", turns: 1, reduce: state.hero.classId === "steward-guardian" ? 0.28 : 0.2 });
    state.hero.defBuff = Math.max(state.hero.defBuff || 0, Math.round(state.hero.def * 0.45)); addLog(`${state.hero.name} ตั้งการ์ด ลดดาเมจรอบถัดไป`); ringFx("heroSpriteWrap", "magic"); renderStatuses(); lockActions(true); setTimeout(enemyTurn, 450);
  }

  function usePotion() {
    if (actionLocked) return; const healAmount = 35; state.hero.hp = Math.min(state.hero.maxHp, state.hero.hp + healAmount); popup("heroSpriteWrap", healAmount, "heal"); ringFx("heroSpriteWrap", "heal"); addLog(`${state.hero.name} ใช้ Potion ฟื้น HP ${healAmount}`); if (postActionCheck()) return; lockActions(true); setTimeout(enemyTurn, 450);
  }

  function startBattle(save, floor, setSaveFunc, getClassById) {
    saveFn = setSaveFunc; const heroClass = getClassById(save.classId); const enemy = pickEnemy(floor);
    state = { turn: 0, floor, hero: { ...save, classId: save.classId, skills: heroClass.skills.slice(), statuses: [], defBuff: 0 }, enemy };

    text("battleFloorTitle", `Floor ${floor}`); text("heroBattleName", save.name); text("enemyName", enemy.name);
    text("heroElementBadge", `${save.element} Lv${save.elementLevel}`); text("enemyElementBadge", `${enemy.element} Lv${enemy.elementLevel}`);
    document.getElementById("heroSprite").src = heroClass.sprite; document.getElementById("enemySprite").src = enemy.sprite;
    text("heroPassiveText", heroClass.passive); text("enemyTypeLabel", enemy.type === "boss" ? "Boss Encounter" : enemy.type === "mini" ? "Mini Boss" : "Standard Encounter");
    document.getElementById("bossBanner").classList.toggle("hidden", enemy.type === "normal"); text("bossBanner", enemy.type === "boss" ? "Boss Battle" : "Mini Boss");
